...
function highlightMatchingOption(selectElementId, targetValue) {
	
		const selectElement = document.getElementById(selectElementId);
	  
		if (!selectElement) {
		  alert('Select element with ID ' + selectElementId + ' not found.');
		  return;
		}
		
		for (let i = 0; i < selectElement.options.length; i++) {
				
		  if (selectElement.options[i].value.trim().toLowerCase() === targetValue.trim().toLowerCase() ) {
...
function LocalDisplayVals()
{
		...
		//document.getElementById('txt_FIRWavFile').value = SqueezeDSPData.FIRWavFile;
		DisplayVal('edt_NewPreset', SqueezeDSPData.Last_preset);
		highlightMatchingOption('sel_FIRWavFile',SqueezeDSPData.FIRWavFile)
    ...
    if ( EQRecords[i].startsWith('Filter') )
    {
        ...
        if (EQSettings[2] == 'ON')
            ...
            // should have got an EQ setting
                ...
                if (FilterType == 'LS' || FilterType == 'LSC')
                {
                ...
                if (FilterType == 'HS' || FilterType == 'HSC')
                {
